...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    ...
    setProfileForm({
      ...
      experienceLevel:
        profile.workoutPreferences?.experienceLevel ?? 'beginner',
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('โหลดการตั้งค่าไม่สำเร็จ กรุณาลองใหม่');
    } finally {
      setLoading(false);
    }
  ...
  async function run(
    ...
    try {
      ...
      if (syncReminders && session?.access_token) {
        ...
        setData(snapshot);
        try {
          await syncLocalReminders(snapshot.reminders);
        } catch {
          setMessage(
            'บันทึกในบัญชีแล้ว แต่ยังไม่ได้รับสิทธิ์แจ้งเตือนบนอุปกรณ์',
          );
          return;
        }
      } else await load();
  ...
  async function chooseUnits(units: Units) {
    if (!session?.access_token) return;
    await run(
      () => saveUnits(session.access_token, units),
      'บันทึกหน่วยวัดแล้ว',
    );
  }
  ...
  async function saveProfileForm(success: string) {
    ...
    if (
      ...
      (profileForm.birthDate &&
        !/^\d{4}-\d{2}-\d{2}$/.test(profileForm.birthDate)) ||
      (heightCm !== null &&
  ...
  const choices = <T extends string>(
    ...
    values.map((item) => (
      ...
      >
        <View
          className={`mr-3 h-5 w-5 rounded-full border-2 ${selected === item.value ? 'border-[6px] border-emerald-600' : 'border-slate-300'}`}
        />
  ...
  async function saveTargets() {
    if (!session?.access_token) return;
    const values = {
      calories: numeric(targets.calories),
      protein_g: numeric(targets.protein_g),
      carbs_g: numeric(targets.carbs_g),
      fat_g: numeric(targets.fat_g),
    };
    if (
      !Number.isInteger(values.calories) ||
      values.calories < 500 ||
      Object.values(values).some(
        (value) => !Number.isFinite(value) || value < 0,
      )
    ) {
      setError('กรุณากรอกเป้าหมายให้ถูกต้อง (พลังงานอย่างน้อย 500 kcal)');
      return;
    }
    await run(
      () => saveNutritionTargets(session.access_token, values),
      'บันทึกเป้าหมายแล้ว',
    );
  }
  ...
  async function addReminder() {
    if (!session?.access_token) return;
    if (
      !/^([01]\d|2[0-3]):[0-5]\d$/.test(newReminderTime) ||
      !newReminderTitle.trim()
    ) {
      setError('กรุณากรอกชื่อและเวลาในรูปแบบ HH:MM');
      return;
    }
    await run(
      async () => {
        await createReminder(session.access_token, {
          type: 'meal',
      ...
      'เพิ่มและตั้งเวลาการแจ้งเตือนแล้ว',
      true,
    );
  }
  ...
  async function toggleReminder(item: ReminderRecord) {
    ...
    await run(
      async () => {
        await updateReminder(session.access_token, {
          ...item,
          enabled: !item.enabled,
        });
      },
      'อัปเดตเวลาการแจ้งเตือนแล้ว',
      true,
    );
  }
  ...
  return (
    <Screen
      title={titles[section] ?? 'การตั้งค่า'}
      subtitle="ค่าที่บันทึกจะซิงก์กับบัญชีของคุณ"
    >
      <Pressable
        ...
        <>
          <Card>
            <View className="gap-3">
              ...
              <TextInput
                className="min-h-12 rounded-xl border border-slate-300 px-3 text-slate-900"
                value={profileForm.displayName}
                onChangeText={(displayName) =>
                  setProfileForm((old) => ({ ...old, displayName }))
                }
              />
              <Text className="text-sm font-medium text-slate-700">
                วันเกิด (YYYY-MM-DD)
              ...
              <TextInput
                ...
                onChangeText={(birthDate) =>
                  setProfileForm((old) => ({ ...old, birthDate }))
                }
              />
              <Text className="text-sm font-medium text-slate-700">
                ส่วนสูง (ซม.)
              ...
              <TextInput
                ...
                onChangeText={(heightCm) =>
                  setProfileForm((old) => ({ ...old, heightCm }))
                }
              />
              ...
              {choices(
                ...
                (activityLevel) =>
                  setProfileForm((old) => ({ ...old, activityLevel })),
              )}
            </View>
          </Card>
          <ActionButton
            label={saving ? 'กำลังบันทึก...' : 'บันทึกข้อมูลส่วนตัว'}
            disabled={saving}
            onPress={() => void saveProfileForm('บันทึกข้อมูลส่วนตัวแล้ว')}
          />
        </>
      ) : null}
      {!loading && section === 'goal' ? (
        <>
          <Card>
            <Text className="font-semibold text-slate-900">เป้าหมายหลัก</Text>
            {choices(
              [
              ...
              profileForm.goalType,
              (goalType) => setProfileForm((old) => ({ ...old, goalType })),
            )}
            <ActionButton
              label="บันทึกเป้าหมายหลัก"
              disabled={saving}
              onPress={() => void saveProfileForm('บันทึกเป้าหมายหลักแล้ว')}
            />
          </Card>
          <Card>
            <View className="gap-3">
              {input('calories', 'พลังงานต่อวัน', 'kcal')}
              {input('protein_g', 'โปรตีน', 'g')}
              {input('carbs_g', 'คาร์โบไฮเดรต', 'g')}
              {input('fat_g', 'ไขมัน', 'g')}
            </View>
          </Card>
          <ActionButton
            label={saving ? 'กำลังบันทึก...' : 'บันทึกเป้าหมาย'}
            disabled={saving}
            onPress={() => void saveTargets()}
          />
        </>
      ) : null}
      {!loading && section === 'units' ? (
        <Card>
          <View className="gap-3">
            {(
              [
                ['metric', 'เมตริก (กก./ซม.)'],
                ['imperial', 'อิมพีเรียล (ปอนด์/นิ้ว)'],
              ] as const
            ).map(([value, label]) => (
              <Pressable
                key={value}
                className="min-h-14 flex-row items-center"
                disabled={saving}
                onPress={() => void chooseUnits(value)}
              >
                <View
                  className={`mr-3 h-5 w-5 rounded-full border-2 ${data?.units === value ? 'border-[6px] border-emerald-600' : 'border-slate-300'}`}
                />
      ...
      {!loading && section === 'reminders' ? (
        <>
            ...
            <Card key={item.id}>
              <View className="flex-row items-center gap-3">
                <View className="flex-1 gap-2">
                  <TextInput
                    accessibilityLabel="ชื่อการแจ้งเตือน"
                    ...
                    onChangeText={(title) =>
                      setData((current) =>
                        current
                          ? {
                              ...current,
                              reminders: current.reminders.map((value) =>
                                value.id === item.id
                                  ? { ...value, title }
                                  : value,
                              ),
                  ...
                  />
                  <TextInput
                    accessibilityLabel="เวลาแจ้งเตือน"
                    className="min-h-11 rounded-xl border border-slate-300 px-3 text-slate-900"
                    keyboardType="numbers-and-punctuation"
                    value={item.time}
                    onChangeText={(time) =>
                  ...
                  <Text className="text-sm text-slate-500">
                    ทุกวัน · {item.days_of_week.length} วัน/สัปดาห์
                  </Text>
                </View>
                <Switch
                  value={item.enabled}
                  disabled={saving}
                  onValueChange={() => void toggleReminder(item)}
              ...
              <View className="mt-3 flex-row gap-3">
                <Pressable
                  ...
                  onPress={() =>
                    ...
                    void run(
                      () =>
                        updateReminder(session.access_token, item).then(
                          () => undefined,
                        ),
                ...
                >
                  <Text className="font-medium text-emerald-700">บันทึก</Text>
                </Pressable>
                <Pressable
                  className="min-h-11 flex-1 justify-center"
                  onPress={() =>
                    session?.access_token &&
                    void run(
                      () => deleteReminder(session.access_token, item.id),
                      'ลบการแจ้งเตือนแล้ว',
                      true,
                    )
                  }
                >
                  <Text className="font-medium text-red-600">ลบ</Text>
                </Pressable>
              </View>
            </Card>
          ))}
          <Card>
            ...
            <TextInput
              accessibilityLabel="ชื่อการแจ้งเตือนใหม่"
              className="mb-2 min-h-12 rounded-xl border border-slate-300 px-3 text-slate-900"
              value={newReminderTitle}
              onChangeText={setNewReminderTitle}
            />
            <TextInput
              accessibilityLabel="เวลาแจ้งเตือนใหม่"
              className="min-h-12 rounded-xl border border-slate-300 px-3 text-slate-900"
              keyboardType="numbers-and-punctuation"
          ...
          <ActionButton
            label="เพิ่มการแจ้งเตือนทุกวัน"
            disabled={saving}
            onPress={() => void addReminder()}
          />
          <Text className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">
            {Platform.OS === 'web'
              ? 'เวลาแจ้งเตือนบันทึกในบัญชีแล้ว แต่ PWA เวอร์ชันนี้ยังไม่ส่ง Web Push กรุณาเปิดแอปเพื่อตรวจรายการเตือน'
              : 'เวลาแจ้งเตือนบันทึกในบัญชีและซิงก์เป็น Local Notification บนอุปกรณ์นี้เมื่ออนุญาต Notifications'}
          </Text>
        </>
      ) : null}
      {error ? (
      ...
      {message ? (
        <Text className="rounded-xl bg-emerald-50 p-3 text-emerald-700">
          {message}
        </Text>
      ) : null}
      {error ? (
